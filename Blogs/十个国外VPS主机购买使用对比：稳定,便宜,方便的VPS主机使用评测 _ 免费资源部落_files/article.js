...
jQuery(document).ready(function($) {	
	//侧栏随动
	var rollStart = $('.weibo'), //滚动到此区块的时候开始随动
		rollSet = $('.weibo,.group,.feed-mail,.tags'); //添加rollStart之前的随动区块
		
	rollStart.before('<div class="da_rollbox" style="position:fixed;background-color:#fff;width:222px"></div>');
	...
	objWindow.scroll(function() {
		if (objWindow.scrollTop() > offset.top){
			...
			rollBox.show().stop().animate({top:0,paddingTop:15},0);
		} else {
			rollBox.hide().stop().animate({top:0},0);
		}
	});
});
...
$(".feed-mail").slideToggle(100);
}
function show(){
$("#box-float").show(300);
}
function slideToggle1(){
$("#float").slideToggle(100);
}
function toggle(){
$(".post-list").toggle(100);
}
function slide(){
$("#box-float").slideDown(300);
}
...
jQuery(document).ready(function() {
//real-avatar
jQuery('#email').blur(function(){jQuery('#real-avatar .avatar').attr('src','https://secure.gravatar.com/avatar.php?gravatar_id=' + hex_md5(jQuery('#email').val())) + '&size=32&d=identicon&r=G&default=https://secure.gravatar.com/avatar/ad516503a11cd5ca435acc9bb6523536?s=32';jQuery('#Get_Gravatar').fadeOut().html('→ 右边头像 →').fadeIn('slow');});
});
...
$('.comment_body').hover(
	function() {
		$(this).find('.thdrpy').stop(true,true).show();
	},
	function() {
		$(this).find('.thdrpy').stop(true,true).hide();
	}
);
...
jQuery(document).ready(function($) {
	...
	var h = {
		...
		img: function() {
			var a = prompt('请输入图片地址，freehao123.info可免费上传图片', 'http://');
			if (a) {
