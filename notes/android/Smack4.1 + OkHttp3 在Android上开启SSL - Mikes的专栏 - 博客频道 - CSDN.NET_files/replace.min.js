...
{selectext(obj);var ret=temp.replace(/(<p(\s+[^>]+)*>)(((?!<\/p)[\s\S])+)<\/p>/ig,function(pTagAll,tagOpen,tagAttr,subContent){var modifiedContent=subContent.replace(/((<[^>]+>)*)([^<]*)/mig,function(all,tag,lastTag,text){if(!tag){tag='';}
if(!text){text='';}
